<template>
    <div id="comments_section" v-if="comment_data.length > 0">
        <div id="class_comment" v-for="item in comment_data" :key="item.key">
            <div id="profile_img_box_xxz">
                <img id="ssjV66anx" :src="`${$config.apiUrl}${item.profile_img}`" alt="">
            </div>

            <div id="comment_data_card">
                <p id="comment_name">{{ item.name }} <span id="comment_date">{{ item.date }}</span></p>
    ...
    <div style="margin-top: 30px;" v-else>
        ...
        <p id="rrg">No private comments yet!</p>
    </div>
</template>

<script>
export default {
    name: 'StudentPrivateCommentsContainer', // Component name
    ...
    computed: {
        ...
        // Comments for classwork
        comment_data: {
            get(){
                return this.$store.state.dialog.student_private_comment_dialog.current_comments;
            }
        }
        //-------------------------------------------
    }
