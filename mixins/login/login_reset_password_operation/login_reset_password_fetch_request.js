export default {
    methods: {
        // A Fetch request method to reset the users password by replacing the current password with a new password
        login_reset_password_fetch_request(request){
            return new Promise((resolve, reject) => {
                fetch(`${ this.$config.apiUrl }/api/v1/reset-password`, request)
