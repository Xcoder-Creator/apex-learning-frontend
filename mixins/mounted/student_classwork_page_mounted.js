export default {
    methods: {
        // Student classwork page mounted mixin method
        student_classwork_page_mounted(){
            ...
            if (detect_ie() === true){
                ...
                if (!('fetch' in window) || !('AbortController' in window)){
                    ...
                    this.$store.commit('student_streams_page/active_tab/update_active_tab', { tab: 1 }); // Update active tab from vuex
                    ...
                    if (is_logged_in === true){
                        ...
                        if (this.$store.state.user_details.user_details.details.user_role === 'Teacher'){
                            this.page_view = false;
                            this.$router.push('/teacher/classes/'); // Redirect user to the classes home page
                        } else if (this.$store.state.user_details.user_details.details.user_role === 'Student'){
                            ...
                            if (class_code){
                                ...
                                if (class_array.length > 0){
                                    ...
                                    if (class_info.length > 0 && class_info.length === 1){
                                        ...
                                        let all_classworks = this.$store.state.student_classwork_page.classwork_comp.active_classworks;

                                        if (all_classworks[class_code]){
                                            if (all_classworks[class_code].length > 0){
                                                let active_classworks = all_classworks[class_code];
                                                this.$store.commit('student_classwork_page/classwork_comp/update_classwork_array', { value: active_classworks });
                                                this.$store.commit('student_streams_page/stream_comp/update_refresh_data', { value: true }); // Refresh data property from vuex
                                            ...
                                            } else if (all_classworks[class_code].length === 0){
                                                this.student_fetch_classworks(class_code); // Fetch classworks
                                            }
                                        } else {
                                            this.student_fetch_classworks(class_code); // Fetch classworks
                                        }
                        ...
                        if (environment === 'development'){
                            ...
                            if (access_token === 'TEST_TOKEN'){
                                ...
                                if (class_code){
                                    this.student_fetch_class_list(class_code, null, null, null);
                                } else {
