export default {
    methods: {
        // Student people page mounted mixin method
        student_people_page_mounted(){
            ...
            if (detect_ie() === true){
                ...
                if (!('fetch' in window) || !('AbortController' in window)){
                    ...
                    this.$store.commit('student_streams_page/active_tab/update_active_tab', { tab: 2 }); // Update active tab from vuex
                            ...
                            if (class_code){
                                ...
                                if (class_array.length > 0){
                                    ...
                                    if (class_info.length > 0 && class_info.length === 1){
                                        ...
                                        let all_people = this.$store.state.student_people_page.people_comp.active_people;

                                        if (all_people[class_code]){
                                            let active_people = all_people[class_code];

                                            if (active_people.length > 0){
                                                this.$store.commit('student_people_page/people_comp/update_people_data_array', { value: active_people }); // Update people data array from vuex
                                                let teacher_data = this.$store.state.student_people_page.people_comp.active_teacher;
                                                this.$store.commit('student_people_page/people_comp/update_teacher_data_array', { value: teacher_data[class_code] }); // Update teacher data array property from vuex
                                                ...
                                                this.$store.commit('student_people_page/people_comp/update_people_data_fetched', { value: true }); // Classwork fetched property from vuex
                                            } else {
                                                this.$store.commit('student_streams_page/stream_comp/update_loading_linear', { value: true }) // Update loading linear property from vuex
                                                this.$store.commit('student_streams_page/stream_comp/update_refresh_data', { value: false }); // Refresh data property from vuex
