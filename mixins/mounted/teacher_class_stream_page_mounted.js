export default {
    methods: {
        // Teacher class stream page mounted mixin method
        teacher_class_stream_page_mounted(){
            ...
            if (detect_ie() === true){
                ...
                if (!('fetch' in window) || !('AbortController' in window)){
                    ...
                    this.$store.commit('teacher_streams_page/active_tab/update_active_tab', { tab: 0 });
                                        ...
                                        if (active_class_posts){
                                            if (active_class_posts.hasOwnProperty(class_code)){
                                                setTimeout(() => {
                                                    ...
                                                    let all_upcoming_work = this.$store.state.teacher_streams_page.stream_comp.active_upcoming_work_data;
                                                    let current_data = all_upcoming_work[class_code];
                                                    this.$store.commit('teacher_streams_page/stream_comp/update_upcoming_work_from_mounted', { value: current_data });

                                                    this.$store.commit('teacher_streams_page/stream_comp/update_can_create_post', { value: true });

                                                    let active_class_posts = this.$store.state.teacher_streams_page.stream_comp.active_class_posts;
                                                    this.$store.commit('teacher_streams_page/stream_comp/update_posts_array', { value: active_class_posts[class_code] }); // Update posts array from vuex
