export default {
    methods: {
        // Send comment
        async send_comment_operation(comment, class_code, post_id){
                ...
                if (/^ *$/.test(comment)){
                    ...
                    if (comment.length <= 500 && comment.length !== 0){
                        ...
                        if (response.status === true){
                            ...
                            if (data_status.status === 'error'){
                            ...
                            } else if (data_status.status === 'post_not_found'){
                                this.$store.commit('dialog/msg_dialog/update_dialog', { value: false, msg: 'Please wait, Sending comment....' }); // Display the msg dialog
                                ...
                                this.can_comment = true;
                                this.comment_data = '';
                                this.$store.commit('dialog/comment_dialog/update_dialog', { value: false });
                                ...
                                this.$store.commit('dialog/comment_dialog/add_new_comment', { comment_data: comment_data });
                            ...
                            if (route_name === 'student-stream' || route_name === 'teacher-stream'){
                                // Update the err msg dialog through vuex
                                let obj = {
                                    value: true,
