export default {
    methods: {
        ...
        async delete_class_post_operation(post_id, class_code, user_id){
            let delete_class_post = this.$store.state.student_streams_page.student_stream_route_config.delete_class_post;

            if (delete_class_post === true){
                this.delete_post_loading = true; // Delete post loading property
                this.$store.commit('student_streams_page/stream_comp/update_loaded_posts', { value: false });
                this.$store.commit('student_streams_page/stream_comp/update_loading_linear', { value: true });
                ...
                let response = await this.delete_class_post_request(request); // Await response from the backend

                // Analyze the response recieved
                if (response.status === true){
                    let data_status = this.delete_class_post_response(response.result); // Validate the response
                    ...
                    if (data_status.status === 'error'){
                        ...
                        this.$store.commit('student_streams_page/stream_comp/update_loaded_posts', { value: true });
                        this.$store.commit('student_streams_page/stream_comp/update_loading_linear', { value: false });
                        ...
                        this.$store.commit('student_streams_page/stream_comp/remove_deleted_post', { class_code: class_code, post_id: post_id }); 
                        ...
                        this.$store.commit('student_streams_page/stream_comp/update_class_post', { class_code: class_code, post_id: post_id });
                        this.$store.commit('dialog/delete_post_dialog/reset_data'); // Close delete post dialog
                        this.delete_post_loading = false; // Delete post loading property
                        this.$store.commit('student_streams_page/stream_comp/update_loading_linear', { value: false });
                        ...
                        if (active_class_posts.hasOwnProperty(class_code)){
                            if (active_class_posts[class_code].length === 0){
                                this.$store.commit('student_streams_page/stream_comp/update_loaded_posts', { value: 'no_posts' });
