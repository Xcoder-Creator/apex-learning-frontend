export default {
    methods: {
        // Teacher fetch class posts mixin method to fetch all available posts for a particular class
        async teacher_fetch_class_posts(class_code, offset){
            ...
            let response = await this.teacher_fetch_class_posts_request(request); // Await response from the backend

            // Analyze the response recieved
            if (response.status === true){
                let data_status = this.teacher_fetch_class_posts_response(response.result); // Validate the response
                ...
                if (data_status.status === 'error'){
                    ...
                    if (data_status.classwork_available === true){
                        ...
                        this.$store.commit('teacher_streams_page/stream_comp/update_upcoming_work_data', data); // Update upcoming work data
                    ...
                    if (data_status.state === 'posts_available'){
                        ...
                        this.$store.commit('teacher_streams_page/stream_comp/update_posts_array', { value: new_posts }); // Update posts array from vuex
                        let class_posts_array = this.$store.state.teacher_streams_page.stream_comp.posts_array;
                        ...
                        if (active_class_posts){
                            if (active_class_posts.hasOwnProperty(class_code)){
                                this.$store.commit('teacher_streams_page/stream_comp/update_active_class_posts', { class_code: class_code, class_posts: class_posts_array }); // Update posts for a particular class from vuex

                                if (active_offsets){
                                    if (active_offsets.hasOwnProperty(class_code)){
                                        this.$store.commit('teacher_streams_page/stream_comp/update_active_post_offset', { class_code: class_code, offset: data_status.offset }); // Update the offset for a particular classes posts from vuex
                                    } else {
                                        this.$store.commit('teacher_streams_page/stream_comp/update_active_post_offset', { class_code: class_code, offset: data_status.offset }); // Update the offset for a particular classes posts from vuex
                                    }
                                } else {
                                ...
                                this.$store.commit('teacher_streams_page/stream_comp/update_loaded_posts', { value: true }) // Update loaded posts property from vuex
                            ...
                            setTimeout(() => {
                                this.$store.commit('teacher_streams_page/stream_comp/update_refresh_data', { value: true }); // Refresh data property from vuex
                                this.$store.commit('teacher_streams_page/stream_comp/update_can_create_post', { value: true });
                                this.$store.commit('student_streams_page/teacher_stream_route_config/update_fetch_more_posts', { value: true });
                                this.$store.commit('teacher_streams_page/stream_comp/update_spinner', { value: false });
                                this.$store.commit('teacher_streams_page/teacher_stream_route_config/update_delete_class_post', { value: true });
                                this.$store.commit('teacher_streams_page/stream_comp/update_loading_linear', { value: false }) // Update loading linear property from vuex
                            }, 2000);
                        ...
                        if (data_status.total_posts_length === 0) {
                            ...
                            this.$store.commit('teacher_streams_page/stream_comp/update_loaded_posts', { value: 'no_posts' }) // Update loaded posts property from vuex
