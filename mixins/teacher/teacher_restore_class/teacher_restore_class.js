export default {
    methods: {
        // Teacher restore class mixin method to restore a particular class
        async teacher_restore_class(class_code){
            ...
            let response = await this.teacher_restore_class_request(request); // Await response from the backend

            // Analyze the response recieved
            if (response.status === true){
                let data_status = this.teacher_restore_class_response(response.result); // Validate the response
                ...
                if (data_status.status === 'error'){
                    ...
                    if (archived_classes.length > 0){
                        ...
                        let obj = {
                            value: true,
                            msg: 'Error, couldn\'t restore class',
                            err: true
                        }
                    ...
                    this.teacher_fetch_class_list(class_code, true, null, null); // Fetch class list
