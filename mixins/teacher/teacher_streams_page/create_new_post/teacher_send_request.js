export default {
    methods: {
        // Create a new post
        async teacher_send_request(request_obj, class_code, post_type, post_data){
            let response = await this.teacher_create_new_post_request(request_obj); // Await response from the backend

            // Analyze the response recieved
            if (response.status === true){
                let data_status = this.teacher_create_new_post_response(response.result); // Validate the response
                ...
                if (data_status.status === 'error'){
                    ...
                    let active_class_posts = this.$store.state.teacher_streams_page.stream_comp.active_class_posts; // All active class posts from vuex
                    ...
                    if (active_class_posts.hasOwnProperty(class_code)){
                        if (post_type === 'plain_post'){
                            ...
                            this.$store.commit('teacher_streams_page/stream_comp/add_new_post', { class_code: class_code, post: post }); // Add new post
                            this.$store.commit('teacher_streams_page/stream_comp/update_loaded_posts', { value: true }); // Show the loaded posts
                        ...
                        this.$store.commit('teacher_streams_page/stream_comp/update_active_post_offset', { class_code: class_code, offset: 0 }); // Update offset
